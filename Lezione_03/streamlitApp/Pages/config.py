...
@retry(
    ...
    before_sleep=lambda retry_state: st.warning(
        f"Retrying... {retry_state.attempt_number}/7"
    ),
)
...
with st.form("cfg-form"):
    endpoint = st.text_input(
        "Endpoint", value=st.session_state.defaultConfig["endpoint"]
    )
    deployment = st.text_input(
        "Deployment", value=st.session_state.defaultConfig["deployment"]
    )
    model = st.text_input("Model", value=st.session_state.defaultConfig["model"])
    api_version = st.text_input(
        "API Version", value=st.session_state.defaultConfig["api_version"]
    )
    subscription_key = st.text_input(
        "Subscription Key",
        type="password",
        value=st.session_state.defaultConfig["subscription_key"],
    )
